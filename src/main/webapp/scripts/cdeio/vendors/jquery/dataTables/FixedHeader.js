...
FixedHeader.prototype = {
	...
	fnInit: function ( oTable, oInit )
	{
        ...
        that._fnOnWindowResize = function() {
            FixedHeader.fnMeasure();
            that._fnUpdateClones();
            that._fnUpdatePositions();
        };

        jQuery(window).resize(that._fnOnWindowResize);

		/* Get things right to start with */
       that._fnOnWindowResize();
	},
    ...
    fnDestroy: function() {
        var cache = this.fnGetSettings().aoCache, i, len, c;
        for (i = 0, len = cache.length; i < len; i++) {
            c = cache[i];
            c.nWrapper.remove();
            cache.splice(i, 1);
        }
        $(window).unbind('resize', this._fnOnWindowResize);
    }
};
