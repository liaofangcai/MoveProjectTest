...
ColReorder.prototype = {
    ...
    if (this.s.allowResize) {
      	...
      	if ( that.dom.drag === null && that.dom.resize === null)
      	{
            ...
            /**
             * tangrui
             * Fix the problem that the columns won't resize when FixedHeader enabled.
             */
            var index = $('th', $(nThTarget).parent()).index(nThTarget);
            var nRealTh = $('th', nTh.parentNode).eq(index);

          /* are we on the col border (if so, resize col) */
          if (Math.abs(e.pageX - Math.round(offset.left + nLength)) <= 5)
           {
            $(nThTarget).css({'cursor': 'col-resize'});
            $(nRealTh).css({ 'cursor': 'col-resize' });
          }
          else {
            $(nThTarget).css({'cursor': 'pointer'});
            $(nRealTh).css({ 'cursor': 'pointer' });
          }
        }
  		} );
		}
	...
	"_fnMouseMove": function ( e, colResized )
	{
		...
		if (this.dom.resize) {
		  ...
		  ////////////////////////
             ...
             * Fix the problem that the columns won't resize when FixedHeader enabled.
             */
            if ( typeof this.s.dt.oInstance._oPluginFixedHeader != 'undefined' )
            {
                this.s.dt.oInstance._oPluginFixedHeader.fnUpdate();
            }
